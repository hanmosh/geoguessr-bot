...
class SimpleCNN(nn.Module):
    ...
    def forward(self, x):
        x = self.pool(self.relu(self.bn1(self.conv1(x))))  # 3 -> 32  224 -> 112
        x = self.pool(self.relu(self.bn2(self.conv2(x))))  # 32 -> 64 112 -> 56
        x = self.pool(self.relu(self.bn3(self.conv3(x))))  # 64 -> 128 56 -> 28
        x = self.relu(self.bn4(self.conv4(x)))             # 128 -> 256 28 
        x = self.relu(self.bn5(self.conv5(x)))             # 256 -> 256 28 
        x = self.avgpool(x)                      # 28 -> 7
        x = x.view(x.size(0), -1)
